...
var laborAddress = config.org_info.labor.address;
var contract_address = config.contracts.identityManagerAddress;
var privateKey = config.org_info.labor.key
...
async function init(){
    ...
    caOrgClient = await buildCAClient(FabricCAServices, ccpOrg, 'ca.org3.example.com');

    // setup the wallet to cache the credentials of the application user, on the app server locally
    let walletPathOrg = path.join(__dirname, '..', '..', 'wallet', 'labor'); 
...
router.get('/', async (req, res) => {
    if(req.user){
        res.redirect("/appChain/labor/lobby");
    }
    else{
        res.render('appChain/labor/home', {title: "App Chain", 'info':req.flash('info'), "require_signature":require_signature});
    }
});
...
router.post('/login',passport.authenticate('user',{
    failureRedirect: '/appChain/labor'
    }), async function(req,res){
...
router.post('/loginWithMetamask', async function(req,res,next){
    ...
    async function(req,res){
        res.send({url: "/appChain/labor/lobby"});
});
        ...
        if(err){
            ...
            db.all(users, [], (err, urs) => {
                if(err){
                    ...
                    res.render("appChain/labor/lobby", {
...
router.get('/myAgreement', isAuthenticated, async (req, res) => {
    ...
    db.all(agreements, [], (err, agrmts) => {
        if(err){
            ...
            res.render("appChain/labor/myAgreement", {
...
router.get('/myCertificate', isAuthenticated, async (req, res) => {  
    res.render("appChain/labor/myCertificate", {title: "App Chain | My Certificate", 'user':true});
});
router.get('/logout', function(req, res) {
    req.logOut();
    res.redirect('/appChain/labor/');
});
